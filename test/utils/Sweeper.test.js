...
const SweeperMock = artifacts.require('SweeperMock');
...
contract('Sweeper', function (accounts) {
  ...
  it('can sweep native', async function () {
    const sweeperNative = await SweeperMock.new();
    await web3.eth.sendTransaction({ from: admin, to: sweeperNative.address, value: web3.utils.toWei("0.5", "ether") })

    let balanceSweeper = await web3.eth.getBalance(sweeperNative.address);
    assert.equal(
      balanceSweeper.toString(),
      web3.utils.toWei("0.5", "ether").toString(),
    );
    await sweeperNative.sweepNative(admin);
    balanceSweeper = await web3.eth.getBalance(sweeperNative.address);
    assert.equal(
      balanceSweeper,
      0,
    );
